...
def make_messages(query, is_rpg=False, gpt_dict=None, history_ja=None, history_zh=None, **kwargs):
    ...
    user_prompt = "参考以下术语表（可为空，格式为src->dst #备注）：\n" + gpt_dict_raw_text + "\n" + "根据上述术语表的对应关系和备注，结合历史剧情和上下文，以流畅的风格将下面的文本从日文翻译成简体中文：" + query
    messages.append(
        {
            "role": "user",
            "content": user_prompt
...
def send_request(query, api_url=None, is_rpg=False, gpt_dict=None, is_test=False, max_tokens=512, **kwargs):
    if api_url is None:
    ...
    try:
        ...
        data = dict(
            ...
            max_tokens=1 if is_test else int(max_tokens),
...
def translate(query, api_url=None, is_rpg=False, gpt_dict=None, max_tokens=512):
    query = query.strip()
    ...
    if not bool(config['利用上文信息翻译（通常会有一定的效果提升，但会导致变慢）']):
        ...
        if bool(config['fix_degeneration']):
            cnt = 0
            while completion_tokens == int(max_tokens):

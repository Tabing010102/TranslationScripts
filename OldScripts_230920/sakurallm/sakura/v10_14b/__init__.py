...
def send_request(query, max_tokens=512, api_url=None, is_rpg=False, gpt_dict=None, is_test=False, **kwargs):
    if api_url is None:
    ...
    try:
        ...
        data = dict(
            ...
            max_tokens=1 if is_test else max_tokens,
...
def translate(query, max_tokens=512, api_url=None, is_rpg=False, gpt_dict=None):
    query = query.strip()
    ...
    if not bool(config['利用上文信息翻译（通常会有一定的效果提升，但会导致变慢）']):
        output = send_request(query, max_tokens, api_url=api_url, is_rpg=is_rpg, gpt_dict=gpt_dict)
        ...
        if bool(config['fix_degeneration']):
            ...
            while completion_tokens == max_tokens:
                ...
                output = send_request(query, max_tokens, api_url=api_url, frequency_penalty=frequency_penalty, is_rpg=is_rpg, gpt_dict=gpt_dict)
                completion_tokens = output["usage"]["completion_tokens"]
                output_text = output["choices"][0]["message"]["content"]
                cnt += 1
                if cnt == 2:
        ...
        output = send_request(query, max_tokens, api_url=api_url, history_zh=history_prompt, is_rpg=is_rpg, gpt_dict=gpt_dict)
        completion_tokens = output["usage"]["completion_tokens"]
        output_text = output["choices"][0]["message"]["content"]
        ...
        if bool(config['fix_degeneration']):
            ...
            while completion_tokens == max_tokens:
                frequency_penalty += 0.1
                output = send_request(query, max_tokens, api_url=api_url, history_zh=history_prompt, frequency_penalty=frequency_penalty, is_rpg=is_rpg, gpt_dict=gpt_dict)
                completion_tokens = output["usage"]["completion_tokens"]
                output_text = output["choices"][0]["message"]["content"]
                cnt += 1
                if cnt == 3:

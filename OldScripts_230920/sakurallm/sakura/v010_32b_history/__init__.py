...
def make_messages(query, is_rpg=False, gpt_dict=None, history_ja=None, history_zh=None, **kwargs):
    ...
    if not is_rpg:
        messages = [
            {
                "role": "system",
                "content": "你是一个轻小说翻译模型，可以流畅通顺地使用给定的术语表以日本轻小说的风格将日文翻译成简体中文"
                           "，并联系上下文正确使用人称代词，注意不要混淆使役态和被动态的主语和宾语，"
                           "不要擅自添加原文中没有的代词，也不要擅自增加或减少换行。"
            }
        ]
    else:
        messages = [
            {
                "role": "system",
                "content": "你是一个游戏翻译模型，可以流畅通顺地使用给定的术语表以日本RPG游戏的风格将日文翻译成简体中文"
                           "，并联系上下文正确使用人称代词，注意不要混淆使役态和被动态的主语和宾语，"
                           "不要擅自添加原文中没有的代词，不要擅自增加或减少换行，也不要翻译英文。"
            }
        ]
    if history_zh:
        for line_zh in history_zh:
            messages.append({"role": "assistant", "content": line_zh})
...
def send_request(query, api_url=None, is_rpg=False, gpt_dict=None, is_test=False, history_zh=None, **kwargs):
    if api_url is None:
    ...
    messages = make_messages(query, is_rpg=is_rpg, gpt_dict=gpt_dict, history_zh=history_zh, **kwargs)
    try:
        # OpenAI
...
def translate_history(query, api_url=None, is_rpg=False, gpt_dict=None, history_zh=None):
    query = query.strip()
    ...
    if not bool(config['利用上文信息翻译（通常会有一定的效果提升，但会导致变慢）']):
        output = send_request(query, api_url=api_url, is_rpg=is_rpg, gpt_dict=gpt_dict, history_zh=history_zh)
        ...
        if bool(config['fix_degeneration']):
            ...
            while completion_tokens == int(config['max_new_token']):
                ...
                output = send_request(query, api_url=api_url, frequency_penalty=frequency_penalty, is_rpg=is_rpg, gpt_dict=gpt_dict, history_zh=history_zh)
                completion_tokens = output["usage"]["completion_tokens"]
                output_text = output["choices"][0]["message"]["content"]
                cnt += 1
                if cnt == 2:

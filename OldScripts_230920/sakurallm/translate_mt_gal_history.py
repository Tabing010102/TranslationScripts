...
from sakurallm.sakura.v010_32b_history import translate_history as translate_history_v010_32b
...
existed_data = {}
try:
    f = open(f'msg_tr_his{str(HISTORY_LENGTH)}_prog.json', encoding='utf8')
...
def trans_thread(endpoint, thread_id, save_progress=False):
    ...
    while True:
        ...
        # history
        history_zh = []
        for i in range(max(0, cur_idx - HISTORY_LENGTH), cur_idx):
            if ori_data_key_list[i] in result:
                history_zh.append(result[ori_data_key_list[i]])
            else:
                history_zh.append(ori_data_key_list[i])
        rlock.release()
        # judge if is cjk str
        ...
        while True:
            try:
                ...
                # 0.10p1 32b
                cur_result = translate_history_v010_32b(cur_key, api_url=endpoint, gpt_dict=full_dict, history_zh=history_zh)
                break
        ...
        if save_progress and cur_idx % 200 == 0:
            ...
            with open(f'msg_tr_his{str(HISTORY_LENGTH)}_prog.json', 'w', encoding='utf8') as f4:
...
with open(f'msg_tr_his{str(HISTORY_LENGTH)}.json', 'w', encoding='utf8') as f3:
